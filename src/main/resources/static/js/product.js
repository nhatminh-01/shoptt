...
async function searchParam(page) {
    ...
    var category = window.location.search.split('id=')[1];
    var urlimg = 'http://'+urlFirst+'/api/public/imageProByPro?id=';
    var url = ''
    ...
    for (i = 0; i < listproduct.content.length; i++) {
        var mainimg = '';
        main += '<div class="col-xs-6 col-sm-6 col-md-4 col-lg-4 signle-item" >'+
            '<a href="'+linkdetail+'?id='+listproduct.content[i].id+'"><img class="img-item" src="'+listproduct.content[i].imageBanner+'"></a>'+
            '<div class="small-image">'+
            mainimg+
            '</div>'+
...
async function loadSearchCategory() {
    ...
    var url = 'http://'+urlFirst+'/api/public/getcategoryByCateId?id='+id;
    const response = await fetch(url, {
        method: 'GET',
        headers: new Headers({
        })
...
async function loadAProduct() {
    ...
    if(id != null){
        ...
        document.getElementById("pros").innerHTML += product.pros
        document.getElementById("size").innerHTML += product.sizess
        document.getElementById("th").innerHTML += product.th
