...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
    ...
    @Query(value = "select sum(i.total_amount) from Invoice i where extract(MONTH from i.created_date) = ?1 and extract(YEAR from i.created_date) = ?2 and (i.pay_type = 1 or i.trangthai_id = 4)", nativeQuery = true)
    Double calStatiticsOnMonth(Integer month, Integer year);

    @Query("select count(i.id) from Invoice i")
    Double tongDonHang();

    @Query(value = "select * from invoice i where extract(MONTH from i.created_date) = ?1 and extract(YEAR from i.created_date) = ?2 and i.trangthai_id = 1", nativeQuery = true)
